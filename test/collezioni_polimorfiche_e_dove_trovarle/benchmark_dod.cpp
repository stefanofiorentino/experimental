...
struct on_off_light_dod_t { size_t on_off_idx; };
struct dimmable_light_dod_t { size_t on_off_idx; size_t level_idx; };
...
struct world_dod {
  ...
  size_t add_on_off(on_off_t&& o) {
    for (size_t i = 0; i < on_offs.size(); ++i)
      if (!on_offs[i].has_value()) {
        on_offs[i].emplace(std::forward<on_off_t>(o));
        return i;
      }
  ...
  void draw(std::ostream& oss) const {
    ...
    for (const auto& ol : on_off_lights)
      oss << "<on_off_light>\n<is_on>\n" << std::boolalpha
          << on_offs[ol.on_off_idx]->is_on << "\n</is_on>\n</on_off_light>\n";
    for (const auto& dl : dimmable_lights)
      oss << "<dimmable_light>\n<is_on>\n" << std::boolalpha
          << on_offs[dl.on_off_idx]->is_on << "\n</is_on>\n<level>\n"
...
static world_dod build_dod(size_t count) {
  world_dod w;
...
// ---------- DOD helpers ----------
static std::string draw_dod(const world_dod& w) {
...
// ============================================================================
static void BM_DOD_Create(benchmark::State& state, size_t n) {
  for (auto _ : state) {
    auto w = build_dod(n);
...
static void BM_DOD_Draw(benchmark::State& state, size_t n) {
  auto w = build_dod(n);
  for (auto _ : state) {
...
static void BM_DOD_Switch(benchmark::State& state, size_t n) {
  auto w = build_dod(n);
  for (auto _ : state) {
    w.do_switch(false);
    w.do_switch(true);
...
static void BM_DOD_Mixed(benchmark::State& state, size_t n) {
  for (auto _ : state) {
    auto w = build_dod(n);
    std::string out = draw_dod(w);
    w.do_switch(false);
